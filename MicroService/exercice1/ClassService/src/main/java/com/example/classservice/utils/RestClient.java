package com.example.classservice.utils;
...
public class RestClient<T> {
    ...
    public T deleteRequest(Class<T> type) {
        HttpEntity<String> requestEntity = new HttpEntity<>("",httpHeaders);
        ResponseEntity<T> response = template.exchange(urlApi, HttpMethod.DELETE, requestEntity, type);
        if(response.hasBody()) {
            return response.getBody();
        }
        return null;
